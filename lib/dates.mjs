 ...
 */
/**
 * The window this site covers.
 *
 * FROM is a guard against archives: a club page that returned a full 2016
 * preseason, scores and all, looks exactly like a current one. TO is what the
 * site is for — the preseason ends when the competitions start, and a friendly
 * announced for October, or an NBA game staged in Europe in January, carries no
 * label saying so and arrives looking ordinary.
 *
 * Exported because build.mjs enforces it and check.mjs verifies it. Held in two
 * places they were free to disagree, and a window that disagrees with its own
 * check is a check that passes while the data is wrong.
 */
export const PRESEASON_FROM = "2026-07-15";
export const PRESEASON_TO = "2026-09-23";

export function resolveDate(text, seasonStart = "2026-07-15") {
